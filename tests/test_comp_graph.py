...
class TestCompGraph(unittest.TestCase):
    ...
    def ag_test_loss_function(self, input):
        ...
        loss_val = autograd.numpy.mean((z - self.y_true) ** 2.0)
        return loss_val
    ...
    def test_topo_sort(self):
        ...
        nxgraph.add_edges_from([(a, c), (a, d), (b, f), (b, e), (c, f)])
        ...
        assert my_topo in all_topos, "CG topo sort failed"
    ...
    def test_NN(self):
        ...
        loss_val = loss.forward(linear2, self.y_true)

        y_ag = self.ag_test_embed_function(self.x_np)
        loss_val_ag = self.ag_test_loss_function(self.x_np)
        ...
        assert np.allclose(
            result, expected
        ), f"Comparison of embeddings to autograd.numpy failed: {result} != {expected}"

        # checks the loss function
        result = loss_val
        expected = loss_val_ag
        assert np.allclose(
            result, expected
        ), f"Comparison of loss to autograd.numpy failed: {result} != {expected}"
        ...
        assert np.allclose(
            result, expected
        ), f"Comparison of gradient to autograd.numpy failed: {result} != {expected}"
